ingester prometheus_linkerd_grpc_path module {
  ...
  gauge "throughput" {
    ...
    source prometheus "throughput" {
      query = "label_replace(sum by (cluster, workload_ns, dst, rt_route) (increase(route_request_total{direction='inbound', dst=~'.*svc.cluster.local.*', rt_route!~'.*/.*|^$', cluster='$input{cluster}'}[1m])), 'service', '$1', 'dst', '([a-zA-Z0-9-]*){1}.([a-zA-Z-.]*):.*')"
  ...
  gauge "status_success" {
    ...
    source prometheus "status_success" {
      query = "label_replace(sum by (cluster, workload_ns, dst, rt_route) (increase(route_response_total{grpc_status='0', direction='inbound', dst=~'.*svc.cluster.local.*', rt_route!~'.*/.*|^$', cluster='$input{cluster}'}[1m])), 'service', '$1', 'dst', '([a-zA-Z0-9-]*){1}.([a-zA-Z-.]*):.*')"
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
    }
  ...
  latency_histo "latency_histo" {
    ...
    source prometheus "latency" {
      query = "label_replace(sum by (cluster, workload_ns, dst, rt_route, le) (increase(route_response_latency_ms_bucket{direction='inbound', dst=~'.*svc.cluster.local.*', rt_route!~'.*/.*|^$', cluster='$input{cluster}'}[1m])), 'service', '$1', 'dst', '([a-zA-Z0-9-]*){1}.([a-zA-Z-.]*):.*')"
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
    }
