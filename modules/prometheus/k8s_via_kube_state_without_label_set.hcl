...
ingester prometheus_kube_container module {
  ...
  gauge "total_container_restarts" {
    ...
    source prometheus "total_container_restarts" {
      query = <<EOT
      label_replace((sum by (cluster, namespace, pod, container) (kube_pod_container_status_restarts_total{})), 'pod_group', '$1', 'pod', '(\\D+)-(.*)')
      EOT
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
  ...
  gauge "container_in_terminated" {
    ...
    source prometheus "container_in_terminated" {
      query = <<EOT
      label_replace((sum by (cluster, namespace, pod, container) (kube_pod_container_status_terminated_reason{reason!='Completed'})), 'pod_group', '$1', 'pod', '(\\D+)-(.*)')
      EOT
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
  ...
  gauge "container_in_waiting" {
    ...
    source prometheus "container_in_waiting" {
      query = <<EOT
      label_replace((sum by (cluster, namespace, pod, container) (kube_pod_container_status_waiting_reason{reason!='ContainerCreating', reason!='PodInitializing'})), 'pod_group', '$1', 'pod', '(\\D+)-(.*)')
      EOT
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
  ...
  gauge "container_cpu_limit" {
    ...
    source prometheus "container_cpu_limit" {
      query = <<EOT
      label_replace((sum by (cluster, namespace, pod, container) (kube_pod_container_resource_limits{resource='cpu', unit='core'})), 'pod_group', '$1', 'pod', '(\\D+)-(.*)')
      EOT
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
  ...
  gauge "container_memory_limit" {
    ...
    source prometheus "container_memory_limit" {
      query = <<EOT
      label_replace((sum by (cluster, namespace, pod, container) (kube_pod_container_resource_limits{resource='memory', unit='byte'})), 'pod_group', '$1', 'pod', '(\\D+)-(.*)')
      EOT
      join_on = {
        "$output{cluster}" = "$input{cluster}"
      }
